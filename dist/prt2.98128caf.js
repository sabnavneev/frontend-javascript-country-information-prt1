...
})({"6LvOA":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
const searchForm = document.getElementById('country-form');
//eventlistener koppelen aan input field
searchForm.addEventListener('submit', searchCountry);
function searchCountry(e) {
    //voorkom refresh
    e.preventDefault();
    //input opslaan in variabele
    let searchContent = document.getElementById('search-content').value;
    //alle landen ophalen via GET request
    async function fetchCountryInfo(searchContent) {
        try {
            const resultArray = await _axiosDefault.default.get('https://restcountries.com/v2/name/' + searchContent);
            const { data  } = resultArray;
            countryInfo.innerHTML += `
                <p>
                <img src="${data[0].flag}" alt="Flag" class="flag">
                <span class="gochi-font">${data[0].name}</span>
                <br>
                <span>${data[0].name} is situated in ${data[0].subregion}. It has a population of ${data[0].population} people.</span>
                <br>
                <span>The capital is ${data[0].capital} and you can pay with ${data[0].currencies[0].name}'s.</span>
                <br>
                </p>
                `;
        } catch (e) {
    ...
    return fetchCountryInfo(searchContent);
}
...
},{}]},["6LvOA","iKN9E"], "iKN9E", "parcelRequirecb08")

//# sourceMappingURL=prt2.98128caf.js.map
